...
docker compose up -d db redis new_redis

# Give redis some time to load data
sleep 2

# Empty redis DBs in case contents are left over from
# a previous run
docker compose exec redis redis-cli flushall
docker compose exec new_redis redis-cli flushall
...
# Migrate app keys to new redis, remove the namespace and delete cache
docker compose run -T --rm --env NEW_REDIS_URL="redis://new_redis:6379/0" web bundle exec rails runner - < ../move.rb

# Check that cache keys are gone
rails_runner check_cache_redis_empty.rb
check_failure "Cache keys are still present in old redis"

# Use a config with new redis and _without_ namespaces
cp .env.new_redis_no_namespace .env.production
...
# Check that app redis key is still available.
rails_runner check_app_redis.rb
